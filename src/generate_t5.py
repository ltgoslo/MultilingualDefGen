...
def load_data(path_to_data, dummy, split="test"):
    if path.isfile(path_to_data):
        ...
        if len(df.columns) == 2:
            df.columns = ["word", "example"]
        logger.info(f"Found the following data columns: {df.columns}")
        if dummy:
            logger.info("Replacing empty examples with EMPTY EXAMPLE")
            df.example = df.example.fillna("EMPTY EXAMPLE")
        else:
            logger.info("Dropping usages with empty examples")
    ...
    contexts = [
        ...
        for ctxt, targetword in zip(df.example, df.word)
    ]
...
if __name__ == "__main__":
    ...
    arg(
        ...
        "-nan",
        type=int,
        help="Replace empty examples with a dummy placeholder? If 0, empty examples will be dropped.",
        choices=[0, 1],
        default=0,
    )
    ...
    for task_prefix in task_instructions:
        ...
        input_sentences = []
        for target, context in zip(test_dataframe.word, test_dataframe.Real_Contexts):
