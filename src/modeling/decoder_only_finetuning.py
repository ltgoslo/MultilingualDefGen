...
def compute_metrics(eval_preds):
    ...
    result["BERTScore_precision"] = round(
        (sum(bert_scores["precision"]) / len(bert_scores["precision"])) * 100, 4
    )
    result["BERTScore_recall"] = round(
        (sum(bert_scores["recall"]) / len(bert_scores["recall"])) * 100, 4
    )
    result["BERTScore_f1"] = round(
        (sum(bert_scores["f1"]) / len(bert_scores["f1"])) * 100, 4
    )
    prediction_lens = [
...
def train(args):
    ...
    if args.verbose:
        print(f'-- QLoRa {"enabled" if args.qlora and args.lora else "disabled"} --')
    ...
    if args.verbose:
        print(f'-- LoRa {"enabled" if args.lora else "disabled"} --')
    if args.lora:
    ...
    dev_dataset = dev_dataset.map(formatting_func)
    ...
    for n, p in model.named_parameters():
        if p.requires_grad:
            requires_grad_any = True
            print("TRAINABLE PARAM:", n)
            break
    print("Any trainable params?", requires_grad_any)
...
if __name__ == "__main__":
    parser = HfArgumentParser(
        (ModelArguments, DataTrainingArguments, TrainingArguments, PEFTArguments)
    )
