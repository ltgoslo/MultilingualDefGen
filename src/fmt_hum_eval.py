...
if __name__ == '__main__':
    ...
    parser.add_argument('--lpath', default='CohereLabs-aya-101-defmodtrain_axolotl24st_fi_tarkoittaa.results.tsv.gz.merged.tsv.gz.labels.tsv.gz')
    parser.add_argument('--gold', default='~/PycharmProjects/axolotl24_shared_task/data/finnish/axolotl.test.fi.gold.tsv')
    args = parser.parse_args()
    print(args)
    labels = pd.read_csv(args.lpath, sep='\t')
    ...
    labels = labels.merge(scores, how='left', on='word').dropna()[
        ['word', 'gloss_x', 'gloss_y', 'sense_id', 'orth', 'bleu', 'bertscore',
         'bertscore_p', 'bertscore_r']] # drop not-novel senses
